...
// ── 자를 칸 ───────────────────────────────────────────────────────
void PanelTFT::setClip(int32_t x, int32_t y, int32_t w, int32_t h) {
  _clipX0 = max((int32_t)0, x);
  _clipY0 = max((int32_t)0, y);
  _clipX1 = min((int32_t)_w, x + w);
  _clipY1 = min((int32_t)_h, y + h);
}

void PanelTFT::clearClip() {
  _clipX0 = _clipY0 = 0;
  _clipX1 = _w;
  _clipY1 = _h;
}
...
// 자를 칸이 걸려 있으면 겹치는 만큼만 칠한다.
void PanelTFT::fillRect(int32_t x, int32_t y, int32_t w, int32_t h, uint16_t color) {
  if (!_ready) return;
  const int32_t x0 = max(x, _clipX0), y0 = max(y, _clipY0);
  const int32_t x1 = min(x + w, _clipX1), y1 = min(y + h, _clipY1);
  if (x1 <= x0 || y1 <= y0) return;
  s_canvas->fillRect(x0, y0, x1 - x0, y1 - y0, color);
  markDirty();
}
...
// 지금 폰트로 글자를 그릴 수 있는가. 메트릭을 올릴 메모리가 없으면 false 다.
bool PanelTFT::fontLoaded() {
...
void PanelTFT::drawString(const char* s, int32_t x, int32_t y) {
  if (!_ready || !s || !*s) return;

  VlwFont* font = _cur ? _cur : slotFor(_basicFont);
  ...
  while (*p) {
    ...
    if (gx >= _clipX1) break;                   // 자를 칸 오른쪽을 지났다 — 남은 글자도 마찬가지
    if (gx + g.w <= _clipX0) continue;          // 아직 칸 왼쪽 — 이 글자는 건너뛴다
...
// 화면 끄기/켜기. 원본(TTGO T-Display)은 ST7789 에 DISPOFF(0x28)·DISPON(0x29)을
