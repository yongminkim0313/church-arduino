...
현재 크기: 플래시 3,385,218B(3.39MB) · 앱 칸 4MB / 전역 64,104B.
...
1. **화면 실물** — 색·글자 크기·카드 경계가 3.5" 에서 어떻게 보이는지(로그로는 알 수 없다)
2. **이어지는 일정 줄 터치** — 줄을 눌러 그 일정으로 건너뛰는 것. 줄이 32px 이라 얇을 수 있다
3. **밀리는 속도** — 마퀴가 흐르는 동안 300KB 를 33ms 마다 민다. 느리면 태스크 주기를 늘린다
4. **무동작 5분 뒤 화면 끄기** — `DISPOFF` 뒤 터치로 깨어나 `DISPON` 으로 멀쩡히 돌아오는지
5. **5분 주기 갱신** — 첫 조회는 확인했다. 두 번째 조회부터도 같은 건수가 오는지
