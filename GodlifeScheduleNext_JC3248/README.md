...
```
┌──────────────────────────────────┐ 320×480
│ 다음 일정            12초 전 갱신│ 상태 띠 (누르면 새로고침)
│ 09:30                       D-2  │ 큰 시각 64px / D-day 34px
│ 10월 7일 (화)      2일 3시간 뒤  │ 날짜 / 남은 시간
├──────────────────────────────────┤
│ 유치부 가을 소풍 사전 답사       │ 제목 — 두 줄로 접는다 (24px)
│ 및 준비 모임                     │
│ 유치원 · 준비물 2/3              │ 기관 · 종일 · 준비물 개수
│ 도시락과 돗자리 챙기기, 후문에서 │ 메모 — 두 줄로 접는다
│ 하차하고 정문으로 모일 것        │
│ ● 돗자리  ● 물통  ○ 도시락       │ 준비물 이름 (원본에 없던 것)
├──────────────────────────────────┤
│ 이어지는 일정      눌러서 넘김 ▶ │
│ D-5   10/10(금) 종일  원장 면담  │ 이어지는 일정 — 일정마다 제 줄
...
| 이어지는 일정 줄 | **그 일정으로 바로 건너뛴다** (원본에 없던 것 — 좌표가 있어 생겼다) |
...
(JC3245 판의 `PanelTFT` 와 다른 곳은 이것 하나뿐이다).
...
| `FontNum34` | D-day |
| `FontNum64` | 큰 시각 — "09:30" 이 175px |

숫자 폰트의 글자 집합이 달란트 리더의 것과 다르다 — 시각의 `:` 와 `D-DAY` 가 필요하다.
...
현재 크기: 플래시 3,385,070B(3.39MB) · 앱 칸 4MB / 전역 64,096B.
...
`godlife_jc3248_list.png` · `godlife_jc3248_marquee.gif` 를 만들고, 어느 줄이 넘쳐서
...
- 컴파일 — 3,385,070B(앱 칸 4MB) · 전역 64,096B · 경고 없음
- 배치·줄 접힘·마퀴 넘침 판정 — 호스트 미리보기(위)로 확인. 보드와 같은 VLW 를 써서
  글자 폭이 같다

## 아직 확인하지 못한 것

1. **화면 실물** — 색·글자 크기·카드 경계가 3.5" 에서 어떻게 보이는지
2. **터치 세 자리** — 상태 띠(새로고침) · 이어지는 일정 줄(건너뛰기) · 그 밖(다음)이
   손가락 굵기에 맞게 나뉘어 있는지. 특히 이어지는 일정 줄이 32px 이라 얇을 수 있다
3. **밀리는 속도** — 마퀴가 흐르는 동안 300KB 를 33ms 마다 민다. 느리면 태스크 주기를 늘린다
4. **HTTPS** — 원본과 같은 코드지만 이 보드에서 핸드셰이크가 8초 안에 끝나는지
5. **무동작 5분 뒤 화면 끄기** — `DISPOFF` 뒤 터치로 깨어나 `DISPON` 으로 멀쩡히 돌아오는지
