...
현재 크기: 플래시 3,392,710B(3.39MB) · 앱 칸 4MB / 전역 64,616B.
...
1. **새 생김새의 실물** — 크림빛 바탕과 크레용 색이 3.5" 에서 어떻게 보이는지.
   특히 흰 카드와 크림 바탕의 차이가 옅어 경계가 묻히지는 않는지(로그로는 알 수 없다)
2. **이어지는 일정 딱지 터치** — 딱지를 눌러 그 일정으로 건너뛰는 것. 딱지가 30px 이라
   얇을 수 있다(누르는 칸은 줄 전체라 30px 은 다 받는다)
