...
3. **흐르는 마퀴의 글자가 깜빡였다.** 반쯤 그려진 프레임이 밀려 나간 것이다
   → 한 프레임을 `beginPaint/endPaint` 로 묶었다(위 "줄 스프라이트가 없다" 참고).
     ...
     **실물에서 깜빡임이 사라진 것까지 확인했다**(2026-09-20, 버스 자물쇠까지 얹은
     병합본 3,387,150B). 두 자물쇠가 같이 돌아도 서로 잠기지 않는다
...
3. **오래 두었을 때** — 마퀴·갱신·화면 끄기를 며칠 이어 돌렸을 때도 그대로인지.
   하루 종일 켜 두는 기기라 여기가 마지막 관문이다
   (밀리는 속도는 실물에서 느려짐 없이 흐르는 것을 보았다)
