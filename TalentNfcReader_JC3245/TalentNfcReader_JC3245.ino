// TalentNfcReader_JC3245 — 가이션 JC3248W535C_I_Y 판 NFC 키링 리더 (펀펀포인트)
//
// 손으로 배선하던 TalentNfcReader(ESP32-S3 DevKitC + ILI9341 + XPT2046)를
// 화면·터치가 한 판에 붙어 나오는 기성품 보드로 옮긴 것이다. 하는 일은 같다.
//
...
//   NFC      PN532 를 I2C 로 바깥 커넥터(GPIO17·18)에 문다 — 이것만 손으로 잇는다
//
// ── 원본과 다른 곳 ────────────────────────────────────────────────
//   1) TFT_eSPI 를 쓰지 않는다. 이 화면은 QSPI 라 TFT_eSPI 가 다루지 못한다.
//      Arduino_GFX 로 그리되, 본체 3,800줄을 고치지 않으려고 같은 이름의 껍데기를
//      뒀다(PanelTFT.h/.cpp). 한글 VLW 폰트는 VlwFont 가 직접 읽는다.
//   2) 화면이 커져(240×320 → 320×480) 띠 높이·단추 크기를 그만큼 키웠다.
//   3) 큰 숫자는 내장 폰트 대신 이 화면에 맞게 구운 FontNum34·FontNum64 를 쓴다.
//   4) 터치가 정전식이라 보정(TOUCH_CAL)이 필요 없다.
//   5) 화면 회전은 세로(0)만 된다 — AXS15231B 에 하드웨어 회전이 없다.
//   6) 부저는 기본으로 달지 않는다(PIN_BUZZER -1). 남는 핀에 물리면 그때 켠다.
//
...
// ── 핀 ────────────────────────────────────────────────────────────
// 화면(QSPI)·터치(I2C)·백라이트는 보드에 붙박이라 고를 것이 없다 — 번호는
// PanelTFT.h 에 모아 뒀다(JC_LCD_*, JC_TOUCH_*). 여기서는 우리가 잇는 것만 정한다.
//
// ── 바깥으로 나온 핀 ──
// 이 보드는 1.25mm JST 커넥터로 몇 개만 빼 준다. 그중 3.3V·GND 와 함께 나오는
// GPIO17·18 두 개를 PN532 의 I2C 로 쓴다. 화면 터치도 I2C(GPIO4·8)지만 그쪽은
// 보드 안에서 이미 물려 있어, PN532 는 **두 번째 버스(Wire1)** 로 따로 연다 —
// 한 버스에 묶으면 터치를 읽을 때마다 PN532 응답을 기다리는 일이 생긴다.
//
//   JST 커넥터        PN532 모듈
//     3V3    ───────   VCC
//     GND    ───────   GND
...
#define PIN_NFC_SDA   17
#define PIN_NFC_SCL   18
// 부저는 기본으로 달지 않는다. 이 보드에서 남는 핀이 두 개뿐이라 NFC 에 다 썼다.
// 스피커 커넥터에 소리를 내려면 증폭기 쪽을 따로 손봐야 해서, 지금은 소리를 끈다
// (-1 이면 beep 가 아무 일도 하지 않는다). 남는 핀에 피에조를 물리면 그 번호를 적는다.
#define PIN_BUZZER    -1
// 백라이트는 보드 붙박이(GPIO1)다.
#define PIN_TFT_BL    JC_LCD_BL
// 딥슬립에서 화면 터치로 깨우는 데 쓰는 핀 — 터치 칩의 INT 다(GPIO3, RTC 핀).
#define PIN_TOUCH_IRQ JC_TOUCH_INT
...
Adafruit_PN532 nfc(-1, -1, &Wire1);   // Wire 는 화면 터치가 쓴다 — PN532 는 Wire1
...
// 코어의 esp32s3 변형은 RGB_BUILTIN 을 GPIO48 로 두는데, JC3248W535 에서 GPIO48 은
// 화면 QSPI 의 데이터선(D1)이다. 거기에 LED 신호를 흘리면 화면이 깨진다.
// (형제 보드인 T-RGB 판에서는 같은 자리가 I2C SCL 이라 터치가 죽었다)
...
void setup() {
  ...
  Wire1.begin(PIN_NFC_SDA, PIN_NFC_SCL);   // 터치(Wire)와 버스를 나눈다
  nfc.begin();
