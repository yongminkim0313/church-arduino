...
CLAUDE.md 의 미결정 항목이라, 지금은 **리더 측 저장(옵션 B)** 으로 두었다.
ESP32 내장 플래시(NVS)에 태그 UID 를 키로 잔액을 저장한다.

- 추가 부품이 필요 없다
- 태그(옵션 A)는 누구나 쓰기 가능해 위변조가 쉽다
- 대신 **리더가 여러 대면 잔액이 각자 따로 쌓인다**. 여러 대를 쓸 계획이면 서버 DB 가 필요하다

바꾸려면 `talentLoad()` / `talentSave()` 두 함수만 갈아끼우면 된다.
