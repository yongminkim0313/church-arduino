// TalentNfcReader_CST820 — LILYGO T-RGB(2.1" 원형) 판 NFC 키링 리더 (펀펀포인트)
//
// 손으로 배선하던 TalentNfcReader(ESP32-S3 DevKitC + ILI9341 + XPT2046)를
// 둥근 화면이 붙어 나오는 기성품 보드로 옮긴 것이다. 하는 일은 같다.
//
//   보드     LILYGO T-RGB — ESP32-S3R8 (플래시 16MB · PSRAM 8MB, qio_opi)
//   화면     2.1" 480×480 원형 IPS, ST7701S, RGB 병렬 (고정 배선)
//            화면을 켜는 명령은 I2C 확장칩(XL9535)을 거쳐 들어간다
//   터치     정전식 CST820 — I2C 0x15 (SDA 8 · SCL 48)
//   NFC      PN532 를 보드의 IIC 커넥터(3V3 · GND · SDA 8 · SCL 48)에 문다
//
// ── 원본과 다른 곳 ────────────────────────────────────────────────
//   1) TFT_eSPI 를 쓰지 않는다. RGB 병렬 화면은 TFT_eSPI 가 다루지 못한다.
//      Arduino_GFX 로 그리되, 본체 3,800줄을 고치지 않으려고 같은 이름의 껍데기를
//      뒀다(PanelTFT.h/.cpp). 한글 VLW 폰트는 VlwFont 가 직접 읽는다.
//   2) **화면이 둥글다.** 원 안에 들어가는 가장 큰 정사각(338×338)을 화면으로 삼고,
//      껍데기가 좌표를 가운데로 옮겨 준다. 네 귀퉁이가 잘리지 않게 하기 위함이다.
//   3) 큰 숫자는 내장 폰트 대신 이 화면에 맞게 구운 FontNum34·FontNum64 를 쓴다.
//   4) 터치가 정전식이라 보정(TOUCH_CAL)이 필요 없다.
//   5) 백라이트가 PWM 이 아니라 16단계 펄스 방식이다 — tft.setBacklight() 로 넣는다.
//   6) 부저는 달지 않는다(PIN_BUZZER -1). 이 보드는 남는 핀이 I2C 커넥터뿐이다.
//
// NTAG-213 키링을 PN532(I2C)로 읽어 화면에 잔액을 보여준다.
// 입력은 화면을 손으로 누르는 것 하나뿐이다.
//
...
//
// 필요 라이브러리: GFX Library for Arduino 1.5 이상, Adafruit PN532 (+ Adafruit BusIO),
//                  NimBLE-Arduino 2.x
// TFT_eSPI 는 더 쓰지 않는다 — 설정 충돌을 신경 쓸 일도 함께 없어졌다.
// PSRAM(opi)을 반드시 켜야 한다. 화면 버퍼 300KB 를 거기에 잡는다.

#include "PanelTFT.h"
#include "FontKR20.h"
#include "FontKR24.h"
#include "FontNum34.h"      // 원본의 내장 폰트 4번 자리(잔액 숫자)
#include "FontNum64.h"      // 원본의 내장 폰트 6번 자리(완료 화면의 큰 숫자)
...
// ── 핀 ────────────────────────────────────────────────────────────
// 화면(RGB 병렬)·터치·백라이트·확장칩은 보드에 붙박이라 고를 것이 없다 — 번호는
// PanelTFT.h 에 모아 뒀다(TR_*). 여기서는 우리가 잇는 것만 정한다.
//
// ── PN532 를 어디에 무는가 ──
// 보드 옆의 1.25mm IIC/UART 커넥터가 3V3 · GND · SDA(GPIO8) · SCL(GPIO48) 넉 선을 빼 준다.
// 이 I2C 버스에는 이미 터치(0x15)와 화면 확장칩(0x20)이 붙어 있지만, PN532 는 0x24 라
// 주소가 겹치지 않는다 — 같은 버스에 그냥 물리면 된다(JC3248 판처럼 버스를 나누지 않는다).
//
//   보드 IIC 커넥터      PN532 모듈
//     3V3    ───────────  VCC
//     GND    ───────────  GND
...
//     GPIO48 ───────────  SCL
//
// PN532 모듈의 스위치는 I2C 자리에 둔다(보통 1:OFF, 2:ON).
#define PIN_NFC_SDA   TR_I2C_SDA
#define PIN_NFC_SCL   TR_I2C_SCL
// 부저는 달지 않는다 — 바깥으로 나온 것이 이 I2C 커넥터뿐이라 자리가 없다.
// (-1 이면 beep 가 아무 일도 하지 않는다)
#define PIN_BUZZER    -1
// 딥슬립에서 화면 터치로 깨우는 데 쓰는 핀 — CST820 의 INT(GPIO1, RTC 핀).
#define PIN_TOUCH_IRQ TR_TOUCH_INT
...
// 터치 보정값 — 이 보드는 정전식이라 보정이 필요 없다. 칩이 화면 좌표를 그대로 준다.
// 원본(XPT2046 저항막)에서 넘어온 자리라 이름만 남겨 둔다(setTouch 는 빈 껍데기다).
...
// 화면 객체(tft)는 PanelTFT.cpp 에 있다 — 여기서는 그 이름을 그대로 쓴다.
...
// 화면이 원본(240×320)보다 커져서(320×480) 같은 px 로 그리면 글자가 작아 보인다.
// 그래서 본체가 부르는 이름은 그대로 두고 실제 폰트만 한 단계씩 올렸다.
//   useFont(14) → 20px (본문·목록)      useFont(20) → 24px (이름·안내문)
// 원본과 같은 크기로 돌리려면 아래 두 줄의 폰트만 바꿔 끼우면 된다.
static uint8_t curFont = 0;
static void useFont(uint8_t px) {
  if (curFont == px) return;
  if (px == 0)       { tft.unloadFont(); curFont = 0; return; }
  if (px >= 20)      { tft.loadFont(FontKR24); curFont = 20; }
  else               { tft.loadFont(FontKR20); curFont = 14; }
}
...
Adafruit_PN532 nfc(-1, -1, &Wire);    // 터치·확장칩과 같은 버스 — 주소가 달라 겹치지 않는다
...
static void beep(uint16_t freq, uint16_t ms) {
  ...
  if (PIN_BUZZER < 0) return;          // 이 보드에는 부저를 달지 않았다(핀 정의 참고)
...
// ※ 이 보드에서는 **내장 LED 를 쓰지 않는다.**
// 코어의 esp32s3 변형은 RGB_BUILTIN 을 GPIO48 로 두는데, T-RGB 에서 GPIO48 은 I2C 의
// SCL 이다. 거기에 LED 신호를 흘리면 버스가 깨져 터치(0x15)·확장칩(0x20)·PN532 가
// 모두 죽는다 — 실기에서 터치가 한 번 응답하고 그 뒤로 NACK 만 나던 원인이 이것이었다.
// 남는 핀에 WS2812 를 달았다면 그 번호를 build 플래그(-DSTATUS_LED_PIN=xx)로 준다.
...
static const int BOOT_BAR_H = 34;
...
static void backlight(bool on) {
  ...
  if (on) {
    for (int d = 0; d < cfg.backlight; d += 15) { tft.setBacklight(d); delay(6); }
    tft.setBacklight(cfg.backlight);
  } else {
    tft.setBacklight(0);
  }
}
...
#define HROW    32   // 내역 한 줄 (글자 20px 기준)
#define HNAV_H  34   // 페이지 띠 높이
...
static void drawHistory() {
  ...
  if (!feedOk) {
    ...
    tft.drawString("내역을 불러오지 못했습니다", tft.width() / 2, contentMid() - 20);
    contentText(inkMuted());
    tft.drawString("연결을 확인하세요", tft.width() / 2, contentMid() + 20);
    useFont(0);
    return;
  }
...
static void drawLowBalance(const char* who, int32_t balance) {
  ...
  tft.setTextDatum(MC_DATUM);
  contentText(inkMain());
  tft.drawString(who, tft.width() / 2, cy - 64);
  useFont(14);
  contentText(inkSpend());
  tft.drawString("포인트가 모자랍니다", tft.width() / 2, cy - 22);
  useFont(0);

  char b[12];
  snprintf(b, sizeof(b), "%ld", (long)balance);
  contentText(inkMuted());
  tft.drawString(b, tft.width() / 2, cy + 34, 4);
}
...
static void drawResultScreen(const char* who, const char* item, int32_t balance, int32_t delta) {
  ...
  tft.setTextDatum(MC_DATUM);
  contentText(inkMain());
  tft.drawString(who, tft.width() / 2, cy - 92);
  ...
  useFont(14);
  contentText(col);
  tft.drawString(line, tft.width() / 2, cy - 56);
  useFont(0);
  ...
  snprintf(b, sizeof(b), "%ld", (long)balance);
  contentText(inkMain());
  tft.drawString(b, tft.width() / 2, cy + 10, 6);

  useFont(14);
  contentText(inkMuted());
  tft.drawString("남은 포인트", tft.width() / 2, cy + 66);
  useFont(0);
}
...
static void drawWhoScreen() {
  ...
  tft.setTextDatum(MC_DATUM);
  contentText(inkMain());
  tft.drawString(whoName[0] ? whoName : whoUid, tft.width() / 2, top + 30);
  useFont(0);
  ...
  if (!whoKnown) {
    useFont(14);
    contentText(inkSub());
    tft.drawString("아직 등록되지 않은 키링입니다", tft.width() / 2, top + 84);
    contentText(inkMuted());
    tft.drawString("지급 탭에서 대면 등록됩니다", tft.width() / 2, top + 118);
    useFont(0);
    return;
  }
  ...
  contentText(inkMain());
  tft.drawString(b, tft.width() / 2, top + 78, 6);

  useFont(14);
  contentText(inkMuted());
  tft.drawString("남은 포인트", tft.width() / 2, top + 126);
  useFont(0);

  if (!whoCount) {
    useFont(14);
    contentText(inkMuted());
    tft.drawString("아직 내역이 없습니다", tft.width() / 2, top + 166);
    useFont(0);
    return;
  }
...
static const int WAIT_PAD   = 26;
static const int WAIT_BAL_W = 118;          // 잔액 자리 폭(내장 4번 폰트로 다섯 자리)
...
static void drawWaitName(int top) {
  ...
  tft.setTextDatum(ML_DATUM);
  contentText(inkMain());
  tft.drawString(curName[0] ? curName : curUid, waitNameX, top + (waitHasPhoto ? 24 : 22));
  useFont(0);
}
...
static void drawWaitBalance(int top) {
  ...
  tft.drawString(b, tft.width() - BORDER - WAIT_PAD, top + 22, 4);
}
...
static void drawWaitList(int top) {
  ...
  if (!shown) {
    tft.setTextDatum(MC_DATUM);
    contentText(inkMuted());
    tft.drawString("등록된 카드가 없습니다", tft.width() / 2, top + 130);
  }
  useFont(0);
}
...
static const int      BIG_BAND_H = 40;
...
static const int BIG_BAL_W = 96;            // 잔액 숫자 자리 폭(내장 4번 폰트 네 자리)
...
static void drawWaitRefresh() {
  ...
  const int top = contentTop();
  restoreContent(tft.width() - BORDER - WAIT_PAD - WAIT_BAL_W, top + 2, WAIT_BAL_W, 44);
  drawWaitName(top);
  drawWaitBalance(top);
  const int listTop = top + 80;
...
static void drawDone() {
  ...
  if (drew) {
    ...
    tft.drawString(amt, tft.width() / 2, y + 20, 4);
    y += 44;
  } else {
    // 그림이 없으면 예전처럼 이름을 띄우고 숫자를 크게 그린다
    useFont(20);
    contentText(inkMain());
    tft.drawString(curName[0] ? curName : curUid, tft.width() / 2, y + 18);
    useFont(0);
    y += 46;
    contentText(inkDir(up));
    tft.drawString(amt, tft.width() / 2, y + 38, 6);
    y += 76;
  }
...
static void afterOnline() {
  ...
  if (cfgFetch()) {
    ...
    tft.setBacklight(cfg.backlight);  // 밝기만 바로 반영(다시 페이드하면 깜빡인다)
  }
...
static void unknownCard(const char* s) {
  ...
  tft.setTextDatum(MC_DATUM);
  contentText(inkSub());
  tft.drawString("처음 보는 카드입니다", tft.width() / 2, contentMid() - 20);
  contentText(inkMuted());
  tft.drawString("관리자 화면에서 등록하세요", tft.width() / 2, contentMid() + 20);
  useFont(0);
...
static void rejectTag(const char* s) {
  ...
  tft.setTextDatum(MC_DATUM);
  contentText(inkSub());
  tft.drawString("펀펀포인트 키링이", tft.width() / 2, contentMid() - 20);
  tft.drawString("아닙니다",         tft.width() / 2, contentMid() + 20);
  useFont(0);
  overlayUntil = millis() + 2200;
  lastActivity = millis();
}
...
static void drawPhotoSync(const char* title, uint16_t done, uint16_t total, const char* name) {
  ...
  tft.setTextDatum(MC_DATUM);
  contentText(inkMain());
  tft.drawString(title, tft.width() / 2, cy - 74);
  useFont(14);
  contentText(inkSub());
  tft.drawString(name && name[0] ? name : " ", tft.width() / 2, cy - 34);

  // 진행 막대
  const int bx = BORDER + 28, bw = tft.width() - bx * 2, by = cy - 6, bh = 16;
  ...
  contentText(inkMuted());
  tft.drawString("화면을 누르면 멈춥니다", tft.width() / 2, contentTop() + contentH() - 24);
  useFont(0);
  char n[16];
...
void setup() {
  ...
  // 백라이트는 화면을 켠 뒤에 다룬다(tft.init 이 핀을 잡는다) — 여기서는 건드리지 않는다.
  ...
  // 글자에 쓸 폰트를 화면 껍데기에 알려 준다. 폰트 데이터(수 MB)는 이 파일만 들고
  // 있는다 — PanelTFT.cpp 에서 같은 헤더를 넣으면 플래시에 두 벌이 들어간다.
  tft.setFonts(FontKR20, FontNum34, FontNum64);
  tft.init();
  // 그림 데이터의 바이트 순서.
  //
  ...
  tft.setRotation(cfg.rotation);   // 이 화면은 세로(0)만 된다 — 그 밖의 값은 무시하고 알린다
  initColors();
  ...
  Wire.begin(PIN_NFC_SDA, PIN_NFC_SCL, 400000);   // tft.init 에서 이미 열었지만 값은 같다
  nfc.begin();
